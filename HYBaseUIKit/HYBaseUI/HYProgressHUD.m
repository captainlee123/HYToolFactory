...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	HYProgressHUD *hud = [[self alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	HYProgressHUD *hud = [self HUDForView:view];
	if (hud != nil) {
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
	NSArray *huds = [HYProgressHUD allHUDsForView:view];
	for (HYProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
+ (void)showLoading:(NSString *)loading {
    [[HYProgressHUD sharedInstance] show:YES];
    [[HYProgressHUD sharedInstance] setMode:HY_MBProgressHUDModeIndeterminate];
    [[HYProgressHUD sharedInstance] setDetailsLabelText:loading];
}
+ (void)showSuccess:(NSString *)success {
    [[HYProgressHUD sharedInstance] show:YES];
    [[HYProgressHUD sharedInstance] hide:YES afterDelay:1.5];
    [[HYProgressHUD sharedInstance] setMode:HY_MBProgressHUDModeCustomViewSuccess];
    [[HYProgressHUD sharedInstance] setDetailsLabelText:[NSString stringWithFormat:@"%@", success]];
}
+ (void)showError:(NSString *)error {
    [[HYProgressHUD sharedInstance] show:YES];
    [[HYProgressHUD sharedInstance] hide:YES afterDelay:1.0];
    [[HYProgressHUD sharedInstance] setMode:HY_MBProgressHUDModeCustomViewWarning];
    [[HYProgressHUD sharedInstance] setDetailsLabelText:[NSString stringWithFormat:@"%@", error]];
}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.animationType = HY_MBProgressHUDAnimationFade;
		self.mode = HY_MBProgressHUDModeIndeterminate;
		self.labelText = nil;
...
//        self.blurView.layer.borderColor = [UIColor lightGrayColor].CGColor;
        ...
        self.blurView.backgroundColor = [UIColor colorWithWhite:0.16 alpha:0.8];
...
- (void)showUsingAnimation:(BOOL)animated {
	if (animated && animationType == HY_MBProgressHUDAnimationZoomIn) {
		self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
	} else if (animated && animationType == HY_MBProgressHUDAnimationZoomOut) {
	...
	if (animated) {
		...
		self.alpha = 1.0f;
		if (animationType == HY_MBProgressHUDAnimationZoomIn || animationType == HY_MBProgressHUDAnimationZoomOut) {
...
- (void)hideUsingAnimation:(BOOL)animated {
	...
	if (animated && showStarted) {
		...
		// in the done method
		if (animationType == HY_MBProgressHUDAnimationZoomIn) {
			self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(1.5f, 1.5f));
		} else if (animationType == HY_MBProgressHUDAnimationZoomOut) {
			self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
		}
...
- (void)updateIndicators {
	...
	BOOL isRoundIndicator = [indicator isKindOfClass:[HY_MBRoundProgressView class]];
	
	if (mode == HY_MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
	...
	else if (mode == HY_MBProgressHUDModeDeterminateHorizontalBar) {
		// Update to bar determinate indicator
		[indicator removeFromSuperview];
        self.indicator = MB_AUTORELEASE([[HY_MBBarProgressView alloc] init]);
		[self addSubview:indicator];
	}
	else if (mode == HY_MBProgressHUDModeDeterminate || mode == HY_MBProgressHUDModeAnnularDeterminate) {
		if (!isRoundIndicator) {
			// Update to determinante indicator
			[indicator removeFromSuperview];
			self.indicator = MB_AUTORELEASE([[HY_MBRoundProgressView alloc] init]);
			[self addSubview:indicator];
		}
		if (mode == HY_MBProgressHUDModeAnnularDeterminate) {
			[(HY_MBRoundProgressView *)indicator setAnnular:YES];
		}
	} 
	else if (mode == HY_MBProgressHUDModeCustomView && customView != indicator) {
		// Update custom view indicator
		[indicator removeFromSuperview];
		self.indicator = customView;
		[self addSubview:indicator];
	} else if (mode == HY_MBProgressHUDModeText) {
		[indicator removeFromSuperview];
		self.indicator = nil;
    } else if (mode == HY_MBProgressHUDModeCustomViewSuccess){
        [indicator removeFromSuperview];
        customView = [[UIImageView alloc] initWithImage:[HYBaseUITool hy_imageNamed:@"hy_hud_success"]];
        customView.bounds = CGRectMake(0, 0, 40, 40);
        self.indicator = customView;
        [self addSubview:indicator];
    }else if (mode == HY_MBProgressHUDModeCustomViewWarning){
        [indicator removeFromSuperview];
        customView = [[UIImageView alloc] initWithImage:[HYBaseUITool hy_imageNamed:@"hy_hud_warning"]];
        customView.bounds = CGRectMake(0, 0, 40, 40);
        self.indicator = customView;
        [self addSubview:indicator];
    }
...
//    }
    ...
    if(self.blur){
        if(!self.blurView){
            self.blurView = [[HYBlurView alloc] initWithFrame:boxRect];
...
@implementation HY_MBRoundProgressView
...
@implementation HY_MBBarProgressView
